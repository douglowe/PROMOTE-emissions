...
!!!!! DL (5/9/2016) quick fix so we can work with input files with no time dimension
   !call read_src_times( ncid, data_file%date, data_file%secs, data_file%ntimes )
	if( allocated( data_file%date ) ) then
		deallocate( data_file%date, data_file%secs )
	end if
	allocate( data_file%date(1), data_file%secs(1), stat=status )
	data_file%date = interp_time%date
	data_file%secs = interp_time%secs
	data_file%ntimes = 1
...
!---------------------------------------------------------------
      ...
      if( status /= nf_noerr )  then
        ...
        if( status /= nf_noerr )  then
          ...
          !call handle_error( status )
!!!!!!!!!! DL (5/9/2016) modifying code to deal with input data with no time dimension          
          write(*,*) '        so we will set the date to the start output time'
          
		  if( allocated( date ) ) then
			deallocate( date, secs )
		  end if
		  allocate( date(1), secs(1), stat=status )
...
!---------------------------------------------------------------
      ...
      integer :: status
      integer :: varid
!!!!!! DL (5/9/2016) modifying code to deal with datasets on lat/lon grid with no time dimension
...
!      ncnt(:) = (/ grid_specs(gndx)%nlons, grid_specs(gndx)%nlats, 1 /)
      nstt(:) = (/ 1, 1 /)
      ncnt(:) = (/ grid_specs(gndx)%nlons, grid_specs(gndx)%nlats /)
